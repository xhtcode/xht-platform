...
const handleClick = () => {
  window.location.href = 'http://127.0.0.1:9000/oauth2/authorize?client_id=oidc-client&response_type=code&scope=openid write&redirect_uri=https://www.baidu.com'
}
onMounted(() => {
  //   getAppList()
})
...
<template>
  <div class="h-full bg-gray-50 p-6 box-border">
    <div class="max-w-6xl mx-auto">
          ...
          <div class="flex-1">
            ...
            <div class="flex flex-wrap gap-4 text-sm text-gray-500">
                            <span class="flex items-center gap-1">
                                <span class="font-medium text-gray-700">账号:</span>
                                {{ userStore.userInfo?.userName || '-' }}
                            </span>
              <span class="flex items-center gap-1">
                                <span class="font-medium text-gray-700">部门:</span>
                                {{ userStore.userInfo?.deptName || '-' }}
                            </span>
              <span class="flex items-center gap-1">
                                <span class="font-medium text-gray-700">手机:</span>
                                {{ userStore.userInfo?.userPhone || '-' }}
                            </span>
              <span class="flex items-center gap-1">
                                <span class="font-medium text-gray-700">注册时间:</span>
      ...
      <div class="mb-4">
        <h3 class="text-xl font-bold text-gray-800">可访问的应用</h3>
        <p class="text-sm text-gray-500 mt-1">点击下方应用卡片即可跳转到对应系统</p>
      </div>

      <el-row :gutter="20" v-loading="appListLoading">
        <el-col
            v-for="app in userStore.appList"
            :key="app.appId"
            :xs="24"
            :sm="12"
            :md="8"
        ...
        >
          ...
          >
            <div class="flex flex-col items-center text-center">
              ...
              <h4 class="text-lg font-bold text-gray-800 mb-2">{{ app.appName }}</h4>
              <p class="text-sm text-gray-500 mb-3 line-clamp-2">{{ app.appDescription }}</p>
