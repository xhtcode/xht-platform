 ...
 */
@Slf4j
public class XhtEnumsSerializableConvert implements Converter<String, XhtEnum<Serializable>> {

    /**
     * 存储枚举值与枚举实例的映射关系
     */
    private final Map<String, XhtEnum<Serializable>> enumValueMap = new ConcurrentHashMap<>();

    /**
     * 存储枚举名称与枚举实例的映射关系
     */
    private final Map<String, XhtEnum<Serializable>> enumNameMap = new ConcurrentHashMap<>();
     ...
     * @param enumType 枚举类型
     */
    public XhtEnumsSerializableConvert(Class<? extends XhtEnum<Serializable>> enumType) {
        // 验证枚举类型
        if (!enumType.isEnum()) {
     ...
     * @param enumType 枚举类型
     */
    private void initializeEnumMaps(Class<? extends XhtEnum<Serializable>> enumType) {
        XhtEnum<Serializable>[] enumConstants = enumType.getEnumConstants();
        if (enumConstants == null || enumConstants.length == 0) {
            log.warn("枚举类型 {} 没有定义任何枚举常量", enumType.getName());
            return;
        }
        for (XhtEnum<Serializable> enumInstance : enumConstants) {
            try {
                // 处理枚举值映射
     ...
     */
    @Override
    public XhtEnum<Serializable> convert(@Nonnull String source) {
