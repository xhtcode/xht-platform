 ...
 * @author xht
 */
class XhtPhoneLoginTokenDeserializer extends JsonDeserializer<XhtPhoneLoginToken> {
     ...
     */
    @Override
    public XhtPhoneLoginToken deserialize(JsonParser jp, DeserializationContext context)
            throws IOException {
        ...
        // 根据认证状态创建相应的token对象
        XhtPhoneLoginToken token = (!authenticated)
                ? XhtPhoneLoginToken.unauthenticated(principal, credentials)
                : XhtPhoneLoginToken.authenticated(principal, credentials, authorities);
